...
@testset "mapreducedims" begin
    ...
    @testset "1D reduce all" begin
        ...
        result = KF.mapreducedims(identity, +, x, 1)
        @test size(result) == (1,)
    ...
    @testset "2D reduce dim 2 (sum along rows)" begin
        x = AT(Float32[1 2 3; 4 5 6])  # shape (2, 3)
        result = KF.mapreducedims(identity, +, x, 2)
    ...
    @testset "3D reduce dim 3" begin
        x = AT(ones(Float32, 4, 5, 6))
        result = KF.mapreducedims(identity, +, x, 3)
    ...
    @testset "3D reduce dims (1, 3)" begin
        x = AT(ones(Float32, 4, 5, 6))
        result = KF.mapreducedims(identity, +, x, (1, 3))
    ...
    @testset "3D reduce all dims" begin
        x = AT(ones(Float32, 4, 5, 6))
        result = KF.mapreducedims(identity, +, x, (1, 2, 3))
    ...
    @testset "5D reduce dim 5" begin
        ...
        result = Array(KF.mapreducedims(identity, +, x_gpu, 5))
    ...
    @testset "5D reduce dims (2, 4)" begin
        ...
        result = Array(KF.mapreducedims(identity, +, x_gpu, (2, 4)))
    ...
    @testset "sum of squares" begin
        x = AT(Float32[1 2; 3 4])
        result = KF.mapreducedims(x -> x^2, +, x, 1)
    ...
    @testset "max reduction" begin
        ...
        result = KF.mapreducedims(identity, max, x, 1)
        @test size(result) == (1, 2)
    ...
    @testset "min reduction" begin
        ...
        result = KF.mapreducedims(identity, min, x, 2)
    ...
    @testset "g post-transform (mean)" begin
        ...
        n = Float32(size(x, 1))
        result = KF.mapreducedims(identity, +, x, 1; g=s -> s / n)
    ...
    @testset "g post-transform (negate)" begin
        x = AT(Float32[1 2; 3 4])
        result = KF.mapreducedims(identity, +, x, 2; g=s -> -s)
    ...
    @testset "mapreducedims! basic" begin
        x = AT(Float32[1 2 3; 4 5 6])
        dst = AT(zeros(Float32, 1, 3))
        KF.mapreducedims!(identity, +, dst, x, 1)
        @test Array(dst) ≈ Float32[5 7 9]
    end

    @testset "mapreducedims! does not allocate dst" begin
        x = AT(ones(Float32, 8, 8))
        dst = AT(zeros(Float32, 8, 1))
        KF.mapreducedims!(identity, +, dst, x, 2)
    ...
    @testset "agrees with Base.mapreduce, 2D dim 1" begin
        ...
        result = Array(KF.mapreducedims(x -> x^2, +, x_gpu, 1))
        @test result ≈ ref
    end
    ...
    @testset "agrees with Base.mapreduce, 2D dim 2" begin
        ...
        result = Array(KF.mapreducedims(identity, +, x_gpu, 2))
        @test result ≈ ref
    end
    ...
    @testset "agrees with Base.mapreduce, 3D dims (1,3)" begin
        ...
        result = Array(KF.mapreducedims(identity, +, x_gpu, (1, 3)))
        @test result ≈ ref
    end
