...
function run_vecmat_benchmarks(n::Int, p::Int; tmp=nothing, FlagType=UInt8)
    ...
    luma_stats = bench("Luma.vecmat!", () -> Luma.vecmat(*, +, x, A; FlagType=FlagType, tmp=tmp))
...
If `df2` is provided, an additional horizontal line is shown above the Luma kernel time
...
function plot_kernel_comparison(
    ...
    for n in n_values
        luma_row = filter(r -> r.n == n && r.method == "Luma", subset)
    ...
    if df2 !== nothing
        ...
        for n in n_values
            luma_row2 = filter(r -> r.n == n && r.method == "Luma", subset2)
