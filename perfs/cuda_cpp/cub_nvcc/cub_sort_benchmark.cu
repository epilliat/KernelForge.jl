// CUB device-radix-sort benchmark — mirrors cub_scan_benchmark.cu's JSON
// envelope so the Julia helper bench_cub_or_nan() can parse the output
// without changes.
//
// Compile:
//   nvcc -O3 -std=c++17 -arch=native cub_sort_benchmark.cu -o bin/cub_sort_benchmark
//
// CLI (matches scan benchmark):
//   ./bin/cub_sort_benchmark -n 100000000 -i 100 -w 500 -t float -j
//   ./bin/cub_sort_benchmark --size 1000000 --type uint32 --json
...
// Generic sort benchmark over CUB's device-level primitives.
//
...
// `M > 1, batched_mode = "segsort"`  : ONE DeviceSegmentedSort::SortKeys
//                                       call (CUB ≥ 1.13). Adaptive: picks
//                                       warp / block / device per segment
//                                       size — the modern batched API.
//
// Each measurement event brackets the full batched workload.
enum class BatchedMode { Single, Loop, Segmented, SegSort };

template <typename T>
BenchmarkResult benchmark_cub_radix_sort(size_t K, int M, BatchedMode mode, float warmup_ms, int num_iterations, const GPUInfo &gpu_info)
{
    ...
    result.operation = M == 1            ? "radix_sort" :
                       ...
                       mode == BatchedMode::SegSort   ? "radix_sort_segsort"   :
                                                        "radix_sort_batched";
    ...
    print_info("\n=== CUB DeviceRadixSort::SortKeys Benchmark ===\n");
    ...
    // Optional segment-offset buffer for the segmented paths.
    const bool needs_segments = (mode == BatchedMode::Segmented || mode == BatchedMode::SegSort) && M > 1;
    int *d_segment_offsets = nullptr;
    if (needs_segments) {
        std::vector<int> h_offsets(M + 1);
        for (int j = 0; j <= M; ++j) h_offsets[j] = j * (int)K;
        CHECK_CUDA(cudaMalloc(&d_segment_offsets, (M + 1) * sizeof(int)));
        CHECK_CUDA(cudaMemcpy(d_segment_offsets, h_offsets.data(),
    ...
    } else if (mode == BatchedMode::SegSort && M > 1) {
        CHECK_CUDA(cub::DeviceSegmentedSort::SortKeys(
            d_temp_storage, temp_storage_bytes,
            d_input, d_output, (int)N, M,
            d_segment_offsets, d_segment_offsets + 1));
    } else {
        // Single-column scratch is sufficient — CUB reuses it across the loop.
    ...
    result.verified = true; // CUB radix is known-correct; skip per-run check.
    ...
    if (d_segment_offsets) CHECK_CUDA(cudaFree(d_segment_offsets));
...
void print_usage(const char *p)
{
    std::cerr << "Usage: " << p << " [OPTIONS]\n\n"
              ...
              << "  --batched-mode MODE   loop | segmented | segsort (default: loop).\n"
              << "                          loop      = M serial DeviceRadixSort::SortKeys calls\n"
              << "                          segmented = one DeviceSegmentedRadixSort::SortKeys call\n"
              << "                          segsort   = one DeviceSegmentedSort::SortKeys call (adaptive)\n"
...
int main(int argc, char **argv)
{
    ...
    for (int i = 1; i < argc; ++i)
    {
        ...
        if (arg == "-h" || arg == "--help") { print_usage(argv[0]); return 0; }
            ...
            else if (m == "segsort")   mode = BatchedMode::SegSort;
    ...
    if (!g_quiet)
    {
        std::cout << "GPU: " << gpu_info.name
                  << "  (sm_" << gpu_info.compute_major << gpu_info.compute_minor << ")\n";
        std::cout << "Peak BW: " << gpu_info.peak_bandwidth_gb_s << " GB/s\n";
    }

    std::vector<BenchmarkResult> results;
    if (dtype == "all" || dtype == "float")  results.push_back(benchmark_cub_radix_sort<float>(N, M, mode, warmup_ms, iterations, gpu_info));
    if (dtype == "all" || dtype == "double") results.push_back(benchmark_cub_radix_sort<double>(N, M, mode, warmup_ms, iterations, gpu_info));
    if (dtype == "all" || dtype == "int")    results.push_back(benchmark_cub_radix_sort<int>(N, M, mode, warmup_ms, iterations, gpu_info));
    if (dtype == "all" || dtype == "uint8")  results.push_back(benchmark_cub_radix_sort<uint8_t>(N, M, mode, warmup_ms, iterations, gpu_info));
    if (dtype == "all" || dtype == "uint32") results.push_back(benchmark_cub_radix_sort<uint32_t>(N, M, mode, warmup_ms, iterations, gpu_info));
    if (dtype == "all" || dtype == "uint64") results.push_back(benchmark_cub_radix_sort<uint64_t>(N, M, mode, warmup_ms, iterations, gpu_info));
