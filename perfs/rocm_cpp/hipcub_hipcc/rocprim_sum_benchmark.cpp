// rocPRIM (via hipCUB) sum-reduction benchmark — AMD analogue of cub_sum_benchmark.cu.
// hipCUB's DeviceReduce::Sum dispatches straight to rocprim::reduce. CLI + JSON
// output are byte-identical to the CUB binary so perfs/julia/bench_utils.jl parses
// both the same way (run_cub_benchmark → results[1].mean_ms/std_ms).
#include <iostream>
#include <vector>
#include <random>
#include <cmath>
...
template <typename T>
BenchmarkResult benchmark_rocprim_sum(size_t N, double warmup_ms, int num_iterations, const GPUInfo &gpu_info)
{
    ...
    CHECK_HIP(hipMalloc(&d_output, sizeof(T)));
    ...
    CHECK_HIP(hipcub::DeviceReduce::Sum(d_temp_storage, temp_storage_bytes, d_input, d_output, N));
    ...
    T h_output;
    CHECK_HIP(hipMemcpy(&h_output, d_output, sizeof(T), hipMemcpyDeviceToHost));
...
void print_help()
{
    std::cout << "rocPRIM (hipCUB) Sum Reduction Benchmark\n\n"
              << "Usage: rocprim_sum_benchmark [options]\n\n"
              << "Options:\n"
              ...
              << "Examples:\n"
              << "  rocprim_sum_benchmark -n 1000000 -j\n"
              << "  rocprim_sum_benchmark --size 100000000 --iterations 200 --json\n"
              << "  rocprim_sum_benchmark -n 10000000 -t float -j\n"
              << "  rocprim_sum_benchmark -n 10000000 -t uint8 -j\n";
}
...
int main(int argc, char *argv[])
{
    ...
    if (!g_quiet)
    {
        std::cout << "=== rocPRIM (hipCUB) Sum Reduction Benchmark ===" << std::endl;
    ...
    if (dtype == "all" || dtype == "uint8")
    {
        ...
        results.push_back(benchmark_rocprim_sum<uint8_t>(N, warmup_ms, iterations, gpu_info));
    }
    ...
    if (dtype == "all" || dtype == "float")
    {
        ...
        results.push_back(benchmark_rocprim_sum<float>(N, warmup_ms, iterations, gpu_info));
    }
    ...
    if (dtype == "all" || dtype == "double")
    {
        ...
        results.push_back(benchmark_rocprim_sum<double>(N, warmup_ms, iterations, gpu_info));
    }
    ...
    if (dtype == "all" || dtype == "int")
    {
        if (!g_quiet)
            std::cout << "\n### Int (32-bit) ###" << std::endl;
        results.push_back(benchmark_rocprim_sum<int>(N, warmup_ms, iterations, gpu_info));
    }
    ...
    if (dtype == "all" || dtype == "uint64")
    {
        ...
        results.push_back(benchmark_rocprim_sum<uint64_t>(N, warmup_ms, iterations, gpu_info));
    }
...
// Compilation command:
// hipcc -O3 -std=c++17 --offload-arch=gfx942 rocprim_sum_benchmark.cpp -o rocprim_sum_benchmark
//
// Usage (new style with flags):
// ./rocprim_sum_benchmark --help
// ./rocprim_sum_benchmark -n 1000000 -j
// ./rocprim_sum_benchmark --size 100000000 --iterations 200 --json
// ./rocprim_sum_benchmark -n 10000000 -t float -j
// ./rocprim_sum_benchmark -n 10000000 -t uint8 -j
...
// ./rocprim_sum_benchmark [N] [iterations] [warmup_ms]
