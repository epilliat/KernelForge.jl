// rocPRIM (via hipCUB) prefix-scan benchmark — AMD analogue of cub_scan_benchmark.cu.
// hipCUB's DeviceScan::InclusiveSum/ExclusiveSum dispatch straight to rocprim::inclusive_scan /
// exclusive_scan. CLI + JSON output are byte-identical to the CUB binary so
// perfs/julia/bench_utils.jl parses both the same way (run_cub_benchmark → results[1].mean_ms).
#include <iostream>
#include <vector>
#include <random>
#include <cmath>
...
template <typename T>
BenchmarkResult benchmark_rocprim_inclusive_scan(size_t N, float warmup_ms, int num_iterations, const GPUInfo &gpu_info)
{
    ...
    print_info("\n=== rocPRIM Inclusive Scan (Cumulative Sum) Benchmark ===\n");
    ...
    CHECK_HIP(hipcub::DeviceScan::InclusiveSum(d_temp_storage, temp_storage_bytes,
                                               d_input, d_output, N));
    ...
    // Verify result
    CHECK_HIP(hipMemcpy(h_output.data(), d_output, N * sizeof(T), hipMemcpyDeviceToHost));
...
template <typename T>
BenchmarkResult benchmark_rocprim_exclusive_scan(size_t N, float warmup_ms, int num_iterations, const GPUInfo &gpu_info)
{
    BenchmarkResult result;
    ...
    print_info("\n=== rocPRIM Exclusive Scan Benchmark ===\n");
    print_info("Data type: ", getTypeNameVerbose<T>(), "\n");
    print_info("Vector size: ", N, " elements\n");
    print_info("Element size: ", sizeof(T), " bytes\n");
    print_info("Memory size (input): ", (N * sizeof(T)) / (1024.0 * 1024.0), " MB\n");
    ...
    CHECK_HIP(hipcub::DeviceScan::ExclusiveSum(d_temp_storage, temp_storage_bytes,
                                               d_input, d_output, N));
    CHECK_HIP(hipMalloc(&d_temp_storage, temp_storage_bytes));
...
int main(int argc, char **argv)
{
    ...
    if (run_inclusive)
    {
        if (dtype == "all" || dtype == "float")
        {
            ...
            results.push_back(benchmark_rocprim_inclusive_scan<float>(N, warmup_ms, iterations, gpu_info));
        }
        ...
        if (dtype == "all" || dtype == "double")
        {
            ...
            results.push_back(benchmark_rocprim_inclusive_scan<double>(N, warmup_ms, iterations, gpu_info));
        }
        ...
        if (dtype == "all" || dtype == "int")
        {
            ...
            results.push_back(benchmark_rocprim_inclusive_scan<int>(N, warmup_ms, iterations, gpu_info));
        }
        ...
        if (dtype == "all" || dtype == "uint64")
        {
            ...
            results.push_back(benchmark_rocprim_inclusive_scan<uint64_t>(N, warmup_ms, iterations, gpu_info));
        }
    }
    ...
    if (run_exclusive)
    {
        if (dtype == "all" || dtype == "float")
        {
            ...
            results.push_back(benchmark_rocprim_exclusive_scan<float>(N, warmup_ms, iterations, gpu_info));
        }
        ...
        if (dtype == "all" || dtype == "double")
        {
            ...
            results.push_back(benchmark_rocprim_exclusive_scan<double>(N, warmup_ms, iterations, gpu_info));
        }
        ...
        if (dtype == "all" || dtype == "int")
        {
            ...
            results.push_back(benchmark_rocprim_exclusive_scan<int>(N, warmup_ms, iterations, gpu_info));
        }
        ...
        if (dtype == "all" || dtype == "uint64")
        {
            ...
            results.push_back(benchmark_rocprim_exclusive_scan<uint64_t>(N, warmup_ms, iterations, gpu_info));
        }
    }
...
// Compilation command:
// hipcc -O3 -std=c++17 --offload-arch=gfx942 rocprim_scan_benchmark.cpp -o bin/rocprim_scan_benchmark
//
// Usage (new style with flags):
// ./rocprim_scan_benchmark --help
// ./rocprim_scan_benchmark -n 1000000 -j
// ./rocprim_scan_benchmark --size 100000000 --iterations 200 --json
// ./rocprim_scan_benchmark -n 10000000 -m both -t float -j
//
// Usage (legacy positional):
// ./rocprim_scan_benchmark [N] [iterations] [warmup_ms] [mode] [dtype]
