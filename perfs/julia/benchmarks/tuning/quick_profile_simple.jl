...
function quick_profile_simple(total_size::Int, ::Type{Op}; arch=KF.RTX1000(), T=Float32, AT, def=false, tuned=true, ms=500) where Op
...
function quick_profile_simple_roc(total_size::Int, ::Type{Op}; arch, T=Float32, AT, def=false, tuned=true, ms=500) where Op
    tuning = Op == KF.MatVec ? matvec_params[T][total_size] : vecmat_params[T][total_size]
    results = []
    ...
    for ((n, p), params) in sort(collect(tuning), by=x -> x[1][1])
        ...
        default_params = KF.resolve_parameters(arch, Op, src)
        @show default_params, Op, size(src), arch
        if def
            t_kf_def_times = bench(backend, kf_def!; ms)
            t_kf_def = minimum(t_kf_def_times)
        else
    ...
    highlighters = Highlighter[]  # v2: Highlighter, not TextHighlighter

    if !isnothing(spdup_col)
        push!(highlighters,
            Highlighter((data, i, j) -> j == spdup_col && data[i, j] isa Number && data[i, j] > 1.0, crayon"green bold"),
            Highlighter((data, i, j) -> j == spdup_col && data[i, j] isa Number && data[i, j] < 1.0, crayon"red"),
        )
    end
    if !isnothing(def_spdup_col)
        push!(highlighters,
            Highlighter((data, i, j) -> j == def_spdup_col && data[i, j] isa Number && data[i, j] > 1.0, crayon"green"),
            Highlighter((data, i, j) -> j == def_spdup_col && data[i, j] isa Number && data[i, j] < 1.0, crayon"red"),
        )
    end
    if tuned || def
        push!(highlighters,
            Highlighter((data, i, j) -> j in 3:6 && data[i, j] isa Tuple && data[i, j][1] != data[i, j][2], crayon"yellow bold"),
        )
    end

    mat = permutedims(reduce(hcat, rows))
    pretty_table(mat;
        header=header,                        # v2: `header` instead of `column_labels`
        highlighters=Tuple(highlighters),     # v2: expects a Tuple, not a Vector
        header_crayon=crayon"bold cyan",      # v2: replaces TextTableStyle
    )
end